...
Home Screen installation uses a base-relative web app manifest and an Apple touch icon. The PNGs are scaled from meka's official artwork with space for the operating system's corner mask. Manifest identity, launch URL, scope, and icon URLs stay within the deployment path. The theme-color meta tag follows the selected theme; installation does not enable offline caching.

The app frame and portaled overlays share safe-area bounds. A Visual Viewport listener adjusts available height when a mobile keyboard opens, and leaves pinch zoom alone. Touch layouts use at least 16px editable text to avoid Safari's automatic focus zoom. Installed iOS apps have separate storage from Safari; completion notifications still depend on the live page receiving events, not a server push subscription.
...
For a new turn observed from an up-to-date history, the first provider event also fetches its saved input when there is no local submission preview. This read starts at the loaded history's end and stops after the first user message, leaving the streamed response intact. A changed revision, turn, or snapshot invalidates an in-flight read. A refresh that retains the same history revision and end position can restart it against the updated prefix; an unreconciled preceding turn prevents it. It never matches messages by text or copies saved output into the live preview.
...
Mermaid renders in a hidden 1200px-wide container so width-dependent charts do not inherit an SVG's 300px default viewport. The resulting SVG is fitted for preview and expansion, including on small screens. Frontmatter is parsed with js-yaml's JSON schema; only a string title is forwarded, serialized anew. Simple bold and italic HTML formatting in quoted flowchart node labels is translated to Mermaid Markdown strings for SVG text rendering. Other label syntax is preserved, and HTML labels remain disabled.
